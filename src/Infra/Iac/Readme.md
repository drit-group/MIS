...
pulumi config set MisSystem.Iac:sql-server-td-db-admin-id
pulumi config set MisSystem.Iac:sql-server-td-db-admin-password --secret

# Provide SMTP server's host, port, user, pass and default email sender.
pulumi config set MisSystem.Iac:default-email-from
pulumi config set MisSystem.Iac:email-server-host
pulumi config set MisSystem.Iac:email-server-port
pulumi config set MisSystem.Iac:email-server-userName
pulumi config set MisSystem.Iac:email-server-password --secret

# Create and provide an identity certificate password
pulumi config set MisSystem.Iac:identity-certificate-password --secret
```
